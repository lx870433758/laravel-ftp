<template>
    <div class="box" v-loading="loading">
        ...
        <el-dialog
            ...
            <el-input v-model="add_dir_name" placeholder="输入文件夹名称"></el-input>
        ...
        <el-dialog
                ...
                width="20%"
                :before-close="handleClose">
            <el-input v-model="add_file_name" placeholder="输入文件名称"></el-input>
            <span slot="footer" class="dialog-footer">
        ...
        <el-table :data="dir_list" style="width: 100%" class="dir_list">
            <el-table-column label="名称" width="180">
                <template slot-scope="scope">
                    <el-button type="text" @click="listToDir(scope.row.name,scope.row.type)"><img
                            v-if="scope.row.type==1" class="file_img" src="/images/dir.jpg"><img class="file_img" v-else
                                                                                                 src="/images/file.jpg">{{scope.row.name}}
                    </el-button>
                </template>
            </el-table-column>
            ...
            <el-table-column label="重命名 / 删除 / 下载" width="180">
                <template slot-scope="scope">
                    <el-button type="primary" size="mini" @click="rename(scope.row.name,scope.row.type)"
                               icon="el-icon-edit"></el-button>
                    <el-button type="danger" size="mini" @click="remove(scope.row.name,scope.row.type)"
                               icon="el-icon-delete"></el-button>
                    <el-button v-if="scope.row.type==0" type="success" size="mini" @click="down(scope.row.name)"
    ...
    export default {
        ...
        methods: {
            ...
            add_file(add_file_name) {
                ...
                account.local_file = '';
                axios({url: '/create_file', method: 'post', data: account}).then((response) => {
                    if (response.data.status == 200) {
                        this.loading = false;
                        this.$message({message: '创建成功', type: 'success'});
